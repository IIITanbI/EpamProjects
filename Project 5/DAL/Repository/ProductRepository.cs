...
namespace DAL.Repository
{
    public class ProductRepository : AbstractRepository, IRepository<Product>, IEnumerable<Product>
    {
        ...
        public void Add(Product item)
        {
            ...
            if (ProductByName(item.Name) != null)
                throw new DalException("This product already exist");

            var rr = Context.ProductSet.Add(ToEntity(item));
            Context.SaveChanges();
            item.Id = rr.Id;
        }
        public void Update(int id, Product item)
        {
            ...
            if (element == null)
                throw new DalException("Product with this ID is not found");
